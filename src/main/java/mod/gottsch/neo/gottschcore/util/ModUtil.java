...
public class ModUtil {
	...
	*/
	...
	public static Optional<ResourceLocation> asLocation(String name) {
		return hasDomain(name) ? Optional.of(ResourceLocation.parse(name)) : Optional.of(ResourceLocation.withDefaultNamespace(name));
	}

	public static ResourceLocation asLocation( String defaultNamespace, String name) {
		return asLocation(name).orElse(ResourceLocation.fromNamespaceAndPath(defaultNamespace, name));
	}
	...
	public static ResourceLocation getName(Block block) {
		...
		ResourceLocation name = BuiltInRegistries.BLOCK.getResourceKey(block).get().location();
		return name;
	}
